...
//   1) app.hud excludes `text` and authored sub-intent fields — the manifest
//      action_metadata declares intent/status/progress/display.
...
function main() {
  ...
  // CURATION: app.hud canonical fields are intent/status/progress/display.
  actionFields['app.hud'] = (actionFields['app.hud'] || []).filter(
    (x) => !['text', 'sub_intent', 'subIntent', 'showSubflow'].includes(x),
  );
