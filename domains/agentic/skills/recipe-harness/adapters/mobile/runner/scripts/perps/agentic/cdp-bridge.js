...
const COMMANDS = {
  ...
  async 'query-ui-target'(client, args, { deviceName } = {}) {
    const specJson = args.join(' ');
    if (!specJson) {
      throw new Error('Usage: query-ui-target <json-spec>');
    }
    const expr = `(function() {
      var spec = ${specJson};
      if (!globalThis.__AGENTIC__?.queryUiTarget) {
        return Promise.resolve({ present: false, visible: false, error: 'queryUiTarget bridge is unavailable' });
      }
      return globalThis.__AGENTIC__.queryUiTarget(spec);
    })()`;
    const result = await cdpEvalAsync(client, expr);
    return { ...result, deviceName };
  },
...
Commands:
  ...
  query-ui-target <json-spec>          Query fiber presence or viewport visibility
