...
function validateVisualClaims(node, issues) {
  ...
  entries.forEach((claim, index) => {
    ...
    if (!(claim.test_id || claim.testId || claim.text_contains || claim.textContains || claim.text)) {
      issues.push(`  [${node.id || '?'}] screenshot claim #${index + 1} requires test_id or text_contains`);
    }
