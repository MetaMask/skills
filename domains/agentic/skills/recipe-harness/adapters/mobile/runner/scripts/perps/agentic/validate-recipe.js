...
function buildWaitForSpec(step) {
  ...
  if (step.test_id) {
    const visibility = step.visibility === 'viewport' ? 'viewport' : 'tree';
    if (visibility === 'viewport' || step.scroll || step.text_contains) {
      const targetSpec = {
        testId: step.test_id,
        textContains: step.text_contains,
        visibility,
      };
      const scrollSpec = step.scroll && typeof step.scroll === 'object'
        ? {
            testId: step.scroll.container_test_id || step.scroll.test_id || step.test_id,
            offset: Number(step.scroll.offset || step.offset || 300),
            animated: step.scroll.animated === true,
          }
        : null;
      const shouldBeVisible = step.visible !== false;
      const expression = `(function() {
        var targetSpec = ${JSON.stringify(targetSpec)};
        var scrollSpec = ${JSON.stringify(scrollSpec)};
        var shouldBeVisible = ${JSON.stringify(shouldBeVisible)};
        if (!globalThis.__AGENTIC__?.queryUiTarget) {
          return Promise.resolve(JSON.stringify({ visible: false, present: false, error: 'queryUiTarget bridge is unavailable' }));
        }
        return globalThis.__AGENTIC__.queryUiTarget(targetSpec).then(function(result) {
          if (scrollSpec && shouldBeVisible && !result.visible && globalThis.__AGENTIC__?.scrollView) {
            globalThis.__AGENTIC__.scrollView(scrollSpec);
          }
          return JSON.stringify(result);
        });
      })()`;
      return {
        expression,
        assert: { operator: 'eq', field: 'visible', value: shouldBeVisible },
        label: `test_id=${step.test_id} visibility=${visibility}`,
        async: true,
      };
    }
    return {
...
function claimToTargetSpec(claim, defaultVisibility = 'viewport') {
  return {
    testId: claim.test_id || claim.testId,
    textContains: claim.text_contains || claim.textContains || claim.text,
    visibility: claim.visibility === 'tree' ? 'tree' : defaultVisibility,
  };
}

async function queryUiTargetForClaim(claim, appRoot, defaultVisibility = 'viewport') {
  const targetSpec = claimToTargetSpec(claim, defaultVisibility);
  const bridgeResult = spawnBridge(appRoot, [
    'query-ui-target',
    JSON.stringify(targetSpec),
  ]);
  return parseRaw(rawResultString(bridgeResult));
}

async function verifyScreenshotClaims(node, appRoot) {
  const claims = node.claims;
  if (!claims || typeof claims !== 'object') return { verified: true, details: [] };

  const details = [];
  for (const claim of normalizeClaimList(claims.must_show)) {
    const result = await queryUiTargetForClaim(claim, appRoot, 'viewport');
    details.push({ type: 'must_show', claim, result });
    if (!result || result.visible !== true) {
      return {
        verified: false,
        details,
        error: `screenshot claim must_show was not visible: ${JSON.stringify(claim)}; result=${JSON.stringify(result)}`,
      };
    }
  }

  for (const claim of normalizeClaimList(claims.must_not_show)) {
    const result = await queryUiTargetForClaim(claim, appRoot, claim.visibility || 'tree');
    details.push({ type: 'must_not_show', claim, result });
    if (result && (result.visible === true || result.present === true)) {
      return {
        verified: false,
        details,
        error: `screenshot claim must_not_show was present/visible: ${JSON.stringify(claim)}; result=${JSON.stringify(result)}`,
      };
    }
  }
...
function resolveArtifactPath(runOptions, recipePath, artifactPath) {
  if (!artifactPath) {
    return '';
  }
...
function readJsonArtifact(runOptions, recipePath, artifactPath) {
  const absolutePath = resolveArtifactPath(runOptions, recipePath, artifactPath);
...
function artifactExists(context, artifactPath) {
  if (!artifactPath) {
    return false;
  }
  ...
  return (
    fs.existsSync(path.join(artifacts.rootDir, artifactPath)) ||
    fs.existsSync(path.join(context.appRoot, artifactPath))
  );
}
...
async function runExecutableNode(node, context, options = {}) {
  ...
  if (node.action === 'command') {
    ...
    const commandResult = spawnSync(node.cmd, {
      cwd: appRoot,
      encoding: 'utf8',
      env: {
        ...process.env,
        RECIPE_ARTIFACT_DIR: artifacts.rootDir,
      },
    ...
    const result = {
      ...
      timedOut: commandResult.error?.code === 'ETIMEDOUT',
    };

    if (node.stdout) {
      const outPath = resolveArtifactPath(runOptions, context.recipePath, node.stdout);
      ensureParentDir(outPath);
      fs.writeFileSync(outPath, combined);
      result.stdoutPath = outPath;
    }
    if (node.stderr) {
      const errPath = resolveArtifactPath(runOptions, context.recipePath, node.stderr);
      ensureParentDir(errPath);
      fs.writeFileSync(errPath, stderr);
      result.stderrPath = errPath;
    }

    const failed =
      commandResult.error ||
      (commandResult.status !== 0 && node.allow_failure !== true);
    if (failed) {
      throw new Error(
        `command failed for step "${node.id || '?'}" with exit ${commandResult.status}` +
          (commandResult.error ? `: ${String(commandResult.error.message || commandResult.error)}` : '')
      );
    }

    stats.passed += 1;
    printPassWithResult(result);
    finalizeNodeRecord(context, node, {
  ...
  if (node.action === 'assert_exit_code') {
    const actual = context.executionContext.last?.exitCode;
    const expected = Number(node.expected);
    if (actual !== expected) {
      throw new Error(
        `assert_exit_code failed for step "${node.id || '?'}": expected ${expected}, got ${actual}`
      );
    }
    const stdoutPath = context.executionContext.last?.stdoutPath;
    const stderrPath = context.executionContext.last?.stderrPath;
    const logText = [stdoutPath, stderrPath]
      .filter(Boolean)
    ...
    if (missing.length > 0 || forbidden.length > 0) {
      throw new Error(
        `assert_exit_code log assertion failed for step "${node.id || '?'}": ` +
          `missing=${JSON.stringify(missing)} forbidden=${JSON.stringify(forbidden)}`
      );
    }

    stats.passed += 1;
    printPassWithResult({ expected, actual, log_must_contain: node.log_must_contain || [] });
    finalizeNodeRecord(context, node, {
      next: node.next || '',
      result: {
        expected,
  ...
  if (node.action === 'assert_json') {
    const parsed = readJsonArtifact(runOptions, context.recipePath, node.path);
    const mismatches = [];
    Object.entries(node.equals || {}).forEach(([field, expected]) => {
    ...
    if (mismatches.length > 0) {
      throw new Error(
        `assert_json failed for step "${node.id || '?'}": ${JSON.stringify(mismatches)}`
      );
    }

    stats.passed += 1;
    printPassWithResult({ path: node.path, equals: node.equals });
    finalizeNodeRecord(context, node, {
      next: node.next || '', result: parsed, startedAt,
    });
    context.currentStepRef.current = null;
    return { next: node.next || '' };
  }

  if (node.action === 'assert_file') {
    if (!artifactExists(context, node.path)) {
      throw new Error(`assert_file failed for step "${node.id || '?'}": ${node.path}`);
    }

    stats.passed += 1;
    printPassWithResult({ path: node.path, exists: true });
    finalizeNodeRecord(context, node, {
      next: node.next || '', result: { path: node.path, exists: true }, startedAt,
    });
    context.currentStepRef.current = null;
    return { next: node.next || '' };
  }

  if (node.action === 'artifact_index') {
    const artifacts = ensureRunArtifacts(runOptions, context.recipePath);
    const manifestPath = path.join(artifacts.rootDir, 'artifact-manifest.json');
    const entries = (node.artifacts || []).map((entry) => {
      const artifactPath = String(entry || '');
      const artifactRootPath = path.isAbsolute(artifactPath)
        ? artifactPath
        : path.join(artifacts.rootDir, artifactPath);
      const appRootPath = path.isAbsolute(artifactPath)
        ? artifactPath
        : path.join(appRoot, artifactPath);
      const resolvedPath = fs.existsSync(artifactRootPath) ? artifactRootPath : appRootPath;
      return {
        path: artifactPath,
        resolvedPath,
    ...
    stats.passed += 1;
    printPassWithResult({ manifestPath, count: entries.length });
    finalizeNodeRecord(context, node, {
      next: node.next || '', result: { manifestPath, artifacts: entries }, startedAt,
    });
    context.currentStepRef.current = null;
    return { next: node.next || '' };
  }
  ...
  if (node.action === 'screenshot') {
    const claimCheck = await verifyScreenshotClaims(node, appRoot);
    if (!claimCheck.verified) {
      throw new Error(claimCheck.error);
    }
    ...
    if (screenshotPath && artifacts && fs.existsSync(screenshotPath)) {
      ...
      if (typeof node.note === 'string' && node.note.trim().length > 0) {
        fs.writeFileSync(
          `${dest}.json`,
          `${JSON.stringify({ note: node.note, nodeId: node.id, claims: node.claims || null, claimCheck }, null, 2)}\n`,
        );
      }
    }
    ...
    console.log('');
    finalizeNodeRecord(context, node, {
      next: node.next || '', result: { screenshot: screenshotPath, claimCheck }, startedAt,
    });
    context.currentStepRef.current = null;
    return { next: node.next || '' };
  }
