        ...
        if (expanded) {
          el.innerHTML =
            ...
            '<div style="display:flex;align-items:center;gap:10px">' +
              '<span id="hud-id" style="color:#00FF88;font-size:18px;font-weight:700;letter-spacing:0.3px;white-space:nowrap;flex:0 0 auto"></span>' +
              '<span id="hud-desc" style="color:#eee;font-size:15px;flex:1 1 0;min-width:0;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical"></span>' +
          ...
          el.innerHTML =
            '<div style="display:flex;align-items:center;gap:10px">' +
              '<span id="hud-id" style="display:none"></span>' +
              '<span id="hud-desc" style="color:#eee;font-size:13px;flex:1 1 0;min-width:0;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical"></span>' +
            '</div>';
        }
      }
...
async function executeWorkflow(doc, ctx) {
  ...
  while (currentNodeId) {
    ...
    ctx.lastActionResult = parseRaw(result.raw);
...
function writeArtifacts(artifactsDir, normalized, trace, passed, failed, totalMs, issueReview) {
  ...
  const captions = {};
  for (const node of Object.values(normalized.workflow?.nodes || {})) {
    if (node.action !== 'screenshot') continue;
    if (typeof node.note !== 'string' || node.note.trim().length === 0) continue;
    const key = node.filename || node.id;
    if (!key) continue;
    captions[key] = node.note;
  }
  if (Object.keys(captions).length > 0) {
    writeFileSync(
      join(artifactsDir, 'screenshots-captions.json'),
...
async function main() {
  ...
  // Bootstrap session. Live recipe runs are CDP-only: the browser/profile
  // lifecycle is prepared outside the runner, and this process attaches to it.
  let sessionManager;
  ...
  if (cdpPort === null) {
    throw new Error(
      'Live recipe runs require CDP. Pass `--cdp-port <port>` or export CDP_PORT.',
    );
  }
  ...
  if (preConditions && preConditions.length > 0) {
    ...
    const preResult = await runPreConditions(preConditions, registries, preContext);
