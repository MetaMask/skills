...
const REGISTRY = {
  'perps.tab_visible': {
    ...
    check: async (_params, context) => {
      let found = false;
      try {
        await waitForTestId(context, 'account-overview__perps-tab', 5000);
        found = true;
      } catch (err) {
        return { pass: false, hint: `Perps tab not visible. Ensure wallet is unlocked and on the home screen. ${err.message || err}` };
      }
      return {
        pass: found === true,
        hint: found ? 'Perps tab is visible' : 'Perps tab not found. Check that the feature is enabled.',
      };
    },
  },
  ...
  'perps.on_market': {
    ...
    check: async (_params, context) => {
      ...
      try {
        ...
        return { pass: false, hint: `Cannot get state: ${err.message || err}` };
      }
      const url = state?.currentUrl;
      const onMarket = url?.includes('/perps/market/') ?? false;
      return {
        pass: onMarket,
        hint: onMarket ? 'On perps market page' : `Expected to be on a perps market page, current URL: ${url ?? 'unknown'}`,
      };
    },
  },
  ...
  'perps.feature_enabled': {
    ...
    check: async (_params, context) => {
      ...
      return {
        ...
        hint: parsed.has ? 'PerpsController present in state' : 'PerpsController not found. Enable PERPS_ENABLED flag and rebuild.',
      };
    },
  },
  ...
  'perps.no_open_position': {
    ...
    check: async (_params, context) => {
      if (!context) return { pass: false, hint: 'No eval context available.' };
      const page = context.getPage();
      const count = await page.locator('[data-testid="perps-close-cta-button"]').count();
      const clear = count === 0;
      return {
        pass: clear,
        hint: clear ? 'No open position on this market' : 'There is an open position. Close it first (use close-position flow).',
      };
    },
  },
