...
function selectorForUiTarget(target = {}) {
  if (target.selector) return String(target.selector);
  if (target.test_id || target.testId) {
    return `[data-testid="${target.test_id || target.testId}"]`;
  }
  return '';
}

async function queryUiTarget(ctx, target = {}) {
  const page = getActivePage(ctx);
  const selector = selectorForUiTarget(target);
  const visibility = target.visibility === 'tree' ? 'tree' : 'viewport';
  let locator = selector ? page.locator(selector).first() : null;
  const textNeedle = target.text_contains || target.textContains || target.text;

  if (!locator && textNeedle) {
    locator = page.getByText(textNeedle, { exact: false }).first();
  }

  if (!locator) {
    return { present: false, visible: false, visibility, error: 'UI target requires selector, test_id, or text_contains' };
  }

  const present = await locator.count().then((count) => count > 0).catch(() => false);
  if (!present) return { present: false, visible: false, visibility, selector };

  if (target.scroll?.strategy === 'into_view' || target.scroll === true) {
    await locator.scrollIntoViewIfNeeded({ timeout: target.timeout_ms || 5000 }).catch(() => {});
    if (target.scroll?.settle_ms) await delay(Number(target.scroll.settle_ms));
  }

  const text = textNeedle ? await locator.textContent({ timeout: 1000 }).catch(() => '') : '';
  const textMatched = textNeedle ? String(text || '').includes(String(textNeedle)) : undefined;
  const visibleByLocator = await locator.isVisible().catch(() => false);

  if (visibility === 'tree') {
    return { present, visible: present, visibility, selector, textMatched };
  }

  const box = visibleByLocator ? await locator.boundingBox().catch(() => null) : null;
  ...
  return {
    present,
    visible: visibleByLocator && inViewport && (textNeedle ? textMatched : true),
    visibility,
    selector,
    textMatched,
    rect: box,
...
async function verifyScreenshotClaims(node, ctx) {
  ...
  for (const claim of toList(claims.must_show)) {
    const result = await queryUiTarget(ctx, { ...claim, visibility: claim.visibility || 'viewport', scroll: claim.scroll });
    details.push({ type: 'must_show', claim, result });
    if (!result.visible) {
      return { verified: false, details, error: `screenshot claim must_show was not visible: ${JSON.stringify(claim)}; result=${JSON.stringify(result)}` };
    }
  }

  for (const claim of toList(claims.must_not_show)) {
    const result = await queryUiTarget(ctx, { ...claim, visibility: claim.visibility || 'tree' });
    details.push({ type: 'must_not_show', claim, result });
    if (result.present || result.visible) {
      return { verified: false, details, error: `screenshot claim must_not_show was present/visible: ${JSON.stringify(claim)}; result=${JSON.stringify(result)}` };
    }
  }
...
async function executeNodeAction(node, ctx) {
  switch (node.action) {
    ...
    case 'screenshot': {
      ...
      return handleAction(
        async () => {
          const claimCheck = await verifyScreenshotClaims(node, ctx);
          if (!claimCheck.verified) {
            throw new Error(claimCheck.error);
          }
          if (settleMs > 0) await delay(settleMs);
          const selector = node.selector || '';
          const shot = await captureScreenshot(ctx, { name: node.filename || node.id, note: node.note || '', selector });
          return { ...shot, claimCheck };
        },
        'screenshot',
      );
    }
...
async function handleWaitFor(node, ctx) {
  ...
  if (node.test_id) {
    if (node.visibility === 'viewport' || node.scroll || node.text_contains) {
      const deadline = Date.now() + timeout;
      let lastResult = null;
      while (Date.now() < deadline) {
        lastResult = await queryUiTarget(ctx, {
          test_id: node.test_id,
          text_contains: node.text_contains,
          visibility: node.visibility || 'viewport',
          scroll: node.scroll || { strategy: 'into_view' },
          timeout_ms: timeout,
        });
        if (Boolean(lastResult.visible) === (node.visible !== false)) {
          return { raw: lastResult, ok: true };
        }
        await delay(poll);
      }
      return { raw: lastResult, ok: false, error: `Timed out waiting for test_id "${node.test_id}" visibility=${node.visibility || 'viewport'}` };
    }
