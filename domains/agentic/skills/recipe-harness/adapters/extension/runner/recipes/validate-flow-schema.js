...
function validateVisualClaims(node, issues) {
  if (node.claims == null) return;
  if (typeof node.claims !== 'object' || Array.isArray(node.claims)) {
    issues.push(`  [${node.id || '?'}] screenshot claims must be an object`);
    return;
  }
  const entries = [
    ...[].concat(node.claims.must_show || []),
    ...[].concat(node.claims.must_not_show || []),
  ].filter(Boolean);
  entries.forEach((claim, index) => {
    if (typeof claim !== 'object' || Array.isArray(claim)) {
      issues.push(`  [${node.id || '?'}] screenshot claim #${index + 1} must be an object`);
      return;
    }
    if (!(claim.test_id || claim.testId || claim.selector || claim.text_contains || claim.textContains || claim.text)) {
      issues.push(`  [${node.id || '?'}] screenshot claim #${index + 1} requires test_id, selector, or text_contains`);
    }
    if (claim.visibility && !['tree', 'viewport'].includes(String(claim.visibility))) {
      issues.push(`  [${node.id || '?'}] screenshot claim #${index + 1} visibility must be tree or viewport`);
    }
  });
}
...
function validateActionShape(node, issues, context = {}) {
  ...
  switch (node.action) {
    case 'command':
      requireFields(node, ['cmd'], issues);
      break;
    case 'assert_json':
      requireFields(node, ['path', 'equals'], issues);
      break;
    case 'assert_file':
      requireFields(node, ['path'], issues);
      break;
    case 'assert_exit_code':
      if (!Object.prototype.hasOwnProperty.call(node, 'expected')) {
        issues.push(`  [${node.id || '?'}] action="assert_exit_code" requires "expected"`);
      }
      break;
    case 'artifact_index':
      if (!Array.isArray(node.artifacts)) {
        issues.push(`  [${node.id || '?'}] action="artifact_index" requires "artifacts" array`);
      }
      break;
    case 'navigate':
    ...
    case 'screenshot': {
      ...
      validateVisualClaims(node, issues);
      break;
    }
    case 'wait_for':
      if (node.visibility && !['tree', 'viewport'].includes(String(node.visibility))) {
        issues.push(`  [${node.id || '?'}] wait_for visibility must be tree or viewport`);
      }
      if ((node.visibility === 'viewport' || node.scroll) && !node.test_id) {
        issues.push(`  [${node.id || '?'}] wait_for viewport/scroll assertions require test_id`);
      }
      if (
        !('assert' in node) &&
        !('route' in node) &&
...
function validateHookSection(sectionName, steps, appRoot, defaultTeam, taskArtifactsDir, issues, seenIds, context = {}) {
...
function validateWorkflowNodes(normalizedDocument, appRoot, defaultTeam, taskArtifactsDir, issues, seenIds, context = {}) {
...
function validateScenario(filePath, registry) {
  ...
  validateWorkflowNodes(normalizedDocument, appRoot, defaultTeam, taskArtifactsDir, issues, seenIds, context);
