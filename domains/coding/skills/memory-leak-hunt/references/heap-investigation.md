...
cycles", "detached nodes accumulate"). A snapshot cannot cheaply prove a *negative*; the
static read does that better. The snapshot's job is to confirm a *suspected* leak and show
its size and retainer chain.
