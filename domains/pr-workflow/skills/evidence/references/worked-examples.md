...
Full runs: claim → lanes → capture → trust-gate → publish. The visual case is in SKILL.md; these cover the non-visual claim shapes.
...
- **Lanes:** A2 `perf_validation` (primary) → D2 chunk membership + C6 CDP netlog (corroborate).
...
- **Lanes:** F1 migration test (primary) → F2 vault round-trip (if the vault is touched).
...
- **Lanes:** B3 regression suite stays green + B4 snapshot diff empty + D1 bundle within noise.
