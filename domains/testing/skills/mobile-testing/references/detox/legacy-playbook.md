...
│  → Open writing-tests.md (spec structure, templates, FixtureBuilder patterns)
│  → If you need POM/selectors: also open page-objects.md
│  → If you need API or feature-flag mocks: also open mocking.md
│  → After writing: run lint/tsc, then open running-tests.md to run and debug
│
├─ Create or update Page Objects / selectors
│  → Open page-objects.md (POM structure, Matchers, Gestures, Assertions, selector conventions)
│  → When writing the spec: open writing-tests.md
│
├─ Mock API or feature flags
│  → Open mocking.md (testSpecificMock, setupRemoteFeatureFlagsMock, setupMockRequest)
│  → When writing the spec: open writing-tests.md
│
...
│  → When wiring a spec: still follow writing-tests.md for `withFixtures` usage
│
└─ Run tests, debug failures, or self-review
   → Open running-tests.md (build check, detox commands, common failures, retry patterns)
```
...
| --------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **Writing or updating a spec**                | [writing-tests.md](writing-tests.md)          | New spec file, spec structure, FixtureBuilder patterns, smoke/regression templates.                                |
| **Page Objects and selectors**                | [page-objects.md](page-objects.md)            | Create or update POM classes, selector/testId conventions, Matchers/Gestures/Assertions API.                       |
| **API and feature flag mocking**              | [mocking.md](mocking.md)                      | testSpecificMock, setupRemoteFeatureFlagsMock, setupMockRequest, shared mock files.                                |
...
| **Running tests, debugging, fixing failures** | [running-tests.md](running-tests.md)          | Build check, detox run commands, lint/tsc, common failures table, retry patterns, iteration loop.                  |
