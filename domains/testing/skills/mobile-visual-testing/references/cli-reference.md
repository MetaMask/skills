...
Launch timeout is 120 seconds. The project CLI does not build MetaMask. Install the intended app on the simulator before launching.

Do not assume other top-level CLI flags exist. For command-line Metro attachment, use `MM_METRO_PORT` as documented below.
...
Start Metro attachment before runtime inspection:
