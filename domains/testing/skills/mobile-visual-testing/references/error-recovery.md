...
It prints a PASS/FAIL report for Xcode, `idb`, `idb_companion`, and a booted simulator, with install commands for anything missing, and exits non-zero when a prerequisite is absent. This is the fastest way to resolve `MM_IOS_DEPENDENCY_MISSING` and `MM_IOS_RUNNER_NOT_READY`.
...
| `MM_LAUNCH_FAILED` | Launch failed. Inspect the simulator, installed app, and `.mm-daemon.log`. |
| `MM_IOS_DEPENDENCY_MISSING` | `idb` (Facebook iOS Debug Bridge) is not installed. It is required to drive the simulator. Run `yarn mm:doctor`, then `brew tap facebook/fb && brew install idb-companion && pip3 install fb-idb`. |
| `MM_IOS_RUNNER_NOT_READY` | Xcode tooling, the simulator, installed app, or requested Metro endpoint is unavailable. Run `yarn mm:doctor`. |
| `MM_IOS_PREREQUISITES_FAILED` | Xcode command-line tools or simulator prerequisites are unavailable. Run `yarn mm:doctor`. |
| `MM_IOS_APP_IDENTITY_MISMATCH` | The app has a different `fox_code` from the expected signature. Reuse the existing installed app or install a matching build outside the mm workflow. |
...
| App identity mismatch | Installed and expected builds have different `fox_code` values | Reuse the existing installed app or install a matching build outside the mm workflow |
| `idb` not installed | Missing iOS Debug Bridge dependency (`MM_IOS_DEPENDENCY_MISSING`) | Run `yarn mm:doctor`, then `brew tap facebook/fb && brew install idb-companion && pip3 install fb-idb` |
...
For `MM_IOS_APP_IDENTITY_MISMATCH`:

1. Prefer launching the already-installed app.
2. If replacing it, build and install a matching app outside of the `mm` workflow (such as through Xcode or standard repository setup scripts).
...
3. Ensure `MM_METRO_PORT` matches the running Metro process.
