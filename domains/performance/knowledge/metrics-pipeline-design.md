...
| **Reporter** | Per-run spans (per-iteration granularity) + aggregated structured logs (for dashboards) |
...
| Per-run snapshots | Sentry spans + `setMeasurement` | Preserves per-iteration granularity for inspection. Sentry is not a store for experimental raw data, so a Mann-Whitney U quality gate needs the per-run samples persisted elsewhere |
