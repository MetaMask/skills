...
Perps review carries a generated phased checklist and a catalog at `references/templates`. Its canonical rules remain in the Perps recipe library. This repository cannot reach that library in CI, so its public check is that the committed `skill.md` and `references/templates/review-pr/static-perps.md` carry the same checklist (`test/perps-review-template.test.mjs`); a hand edit to either fails. The Perps skill maintainers regenerate both from a clean library revision with `node domains/perps/skills/perps-review-pr/scripts/materialize-review.mjs --library <library>` and run it with `--check` on a machine that has the library, before opening the pull request. An explicitly configured analyzer can receive the same content with `--analyzer-out <consumer>/.ai-pr-analyzer/prompt-context.md`. Review that consumer change before enabling it.
